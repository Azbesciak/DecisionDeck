...
#inDirectory <- "/path/to/weightedSum/tests/in1.v2"
#outDirectory <- "/path/to/weightedSum/tests/out_tmp/"
...
weightsFile <- "criteriaWeights.xml"
parametersFile <- "parameters.xml"
overallValuesFile <- "alternativesValues.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, alternativesFile, mandatory = FALSE, xmcdaMessages, "alternatives")
loadXMCDAv3(xmcdaData, inDirectory, criteriaFile, mandatory = FALSE, xmcdaMessages, "criteria")
...
loadXMCDAv3(xmcdaData, inDirectory, parametersFile, mandatory = TRUE, xmcdaMessages, "programParameters")
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate weightedSum.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert weightedSum results into XMCDA")
}
