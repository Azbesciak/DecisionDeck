...
public class Utils {
    ...
    private static XmcdaVersion getXmcdaVersion(String versionString) throws InvalidCommandLineException {
        return Arrays.stream(XmcdaVersion.values())
                .filter(s -> s.versionFlag.equals(versionString))
                .findFirst()
                .orElseThrow(() -> new InvalidCommandLineException(
     ...
     * @param xmcdaVersion       indicates which {@link XmcdaVersion} to use when writing the file
     */
    ...
    /**
     * Calls {@link #writeProgramExecutionResults(File, ProgramExecutionResult, XmcdaVersion)} then exits. The return status code
